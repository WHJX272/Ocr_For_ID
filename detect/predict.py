...
import detect.config as config
from detect.model import CTPN_Model
from detect.utils import nms, TextProposalConnectorOriented
from detect.utils import resize, gen_anchor, bbox_transfor_inv, clip_box, filter_bbox

prob_thresh = 0.5
height = 720
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
weights = os.path.join(config.checkpoints, 'CTPN.pth')
model = CTPN_Model()
model.load_state_dict(torch.load(weights,device)['model_state_dict'])
model.to(device)
model.eval()
...
if __name__ == '__main__':
    img_path = config.images
    img = cv2.imread(img_path)
    text, image, _ = get_det_boxes(img)
    dis(image)
